    ...
    def load(self, identifier, token, revision):
        if self.version != identifier:
            self.model, self.processor = load_model(
                identifier, token, self.device, revision
            )
...
class Florence2:
    ...
    def apply(
        ...
        results, output_image = self.process_image(
            img, task, max_new_tokens, num_beams, do_sample, fill_mask, text_input
        )
    ...
    def run_example(
        self, task_prompt, image, max_new_tokens, num_beams, do_sample, text_input=None
    ):
    ...
    def process_image(
        ...
        if task_prompt == "caption":
            ...
            result = self.run_example(
                task_prompt, image, max_new_tokens, num_beams, do_sample
            )
            ...
            results = self.run_example(
                task_prompt, image, max_new_tokens, num_beams, do_sample, text_input
            )
            ...
            output_image = draw_polygons(
                image, results["<REFERRING_EXPRESSION_SEGMENTATION>"], fill_mask
            )
            ...
            output_image = draw_polygons(
                image, results["<REGION_TO_SEGMENTATION>"], fill_mask
            )
            return results[task_prompt], output_image
            ...
            output_results = {
                "bboxes": results[task_prompt].get("quad_boxes", []),
